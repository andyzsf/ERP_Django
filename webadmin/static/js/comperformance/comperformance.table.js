    ...
    $(document).ready(function() {
        ...
        if( $.fn.dataTable ) {
            ...
            $("#users").dataTable({
                ...
                "sAjaxSource": "/ajax/get_users_list/",
                //"fnServerParams": function(aoData) {
                //    aoData.push({name:"sSearch_4",value:"172.29.142.136",});
                //},
                "fnServerData":function(sSource,aoData,fnCallback){
            ...
            $('#users_filter').attr('style','height:25px;');
        
            $("<a href='#' class='btn' id='edituser' style='float:right;width:66px;margin-right:2px;'><i class='icon-edit'></i>修改</a> \
            <a href='#' class='btn' id='deleteuser'  style='float:right;width:66px;margin-right:2px;'><i class='icon-remove'></i>删除</a> \
            <a href='#' class='btn' id='adduser' style='float:right;width:66px;margin-right:2px;'><i class='icon-plus'></i>添加</a>\
            <a href='#' class='btn' id='inituser' style='float:right;width:100px;margin-right:2px;'><i class='icol-ui-text-field-password'></i>初始化密码</a>").appendTo('#users_filter');
            
            //init one row user
            $("#inituser").bind("click", function (event) {
                event.preventDefault();
                if($user_selectrow == null){
                    $("#select-user-dialog").dialog("open");
                }else{
                    $('#init_user_id').val($($user_selectrow[0]).text());
                    $("#init-user-dialog").dialog("open");
                }
            });
            ...
            $("#deleteuser").bind("click", function (event) {
                ...
                if($user_selectrow == null){
                    ...
                    $('#delete_user_id').val($($user_selectrow[0]).text());
            ...
            $("#edituser").bind("click", function (event) {
                event.preventDefault();
                if($user_selectrow == null){
                    $("#select-user-dialog").dialog("open");
                }else{
                    $('#edit_user_id').val($($user_selectrow[0]).text());
                    $('#editusername').val($($user_selectrow[1]).text());
                    $('#edituseremail').val($($user_selectrow[3]).text());
                    
                    $('#editstudentsex').find('span').each(function(index,element){
                        if($(element).text() == $($user_selectrow[2]).text()){
                            $(this).parent().click();
                        }
                    });
            ...
            $("#customers").dataTable({
                ...
                "bServerSide": true,
                //"sAjaxSource": "/ajax/get_students_list/",
                "sAjaxSource": "/ajax/get_customers_list/",
                //"fnServerParams": function(aoData) {
                //    aoData.push({name:"sSearch_4",value:"172.29.142.136",});
                //},
                "fnServerData":function(sSource,aoData,fnCallback){
                ...
                "oTableTools":{
                    "sRowSelect": "single",             
                    "fnRowSelected":function(node){
                        $customer_selectrow = $(node).children();
            ...
            $('#customers_filter').attr('style','height:25px;');
        
            $("<a href='#' class='btn' id='editcustomer' style='float:right;width:66px;margin-right:2px;'><i class='icon-edit'></i>修改</a> \
            <a href='#' class='btn' id='deletecustomer'  style='float:right;width:66px;margin-right:2px;'><i class='icon-remove'></i>注销</a> \
            <a href='#' class='btn' id='addcustomer' style='float:right;width:66px;margin-right:2px;'><i class='icon-plus'></i>添加</a>").appendTo('#customers_filter');
            
            
            
            //delete one row customer
            $("#deletecustomer").bind("click", function (event) {
                event.preventDefault();
                if($customer_selectrow == null){
                    $("#select-customer-dialog").dialog("open");
                }else{
                    $('#delete_customer_id').val($($customer_selectrow[0]).text());
            ...
            //edit one row customer
            
            $("#editcustomer").bind("click", function (event) {
                event.preventDefault();
                if($customer_selectrow == null){
                    $("#select-customer-dialog").dialog("open");
                }else{
                    $('#editcustid').val($($customer_selectrow[0]).text());
                    $('#editcustcompany').val($($customer_selectrow[1]).text());
                    $('#editlinkman').val($($customer_selectrow[2]).text());
                    $('#editaddress').val($($customer_selectrow[3]).text());
                    $('#editcity').val($($customer_selectrow[4]).text());
                    $('#editregion').val($($customer_selectrow[5]).text());
                    $('#editzipcode').val($($customer_selectrow[6]).text());
                    $('#edittel').val($($customer_selectrow[7]).text());
                    $('#editfax').val($($customer_selectrow[8]).text());
                    $('#edithomepage').val($($customer_selectrow[9]).text());
                    $('#editmessage').val($($customer_selectrow[10]).text());
                    
                    

                    $("#edit-customer-dialog").dialog("open");
                }
            });
            ...
            //selectproduct show
            var $selectproduct_selectrow = null;
            
            $("#selectproducts").dataTable({
                "iDisplayLength": 6,
                "aLengthMenu": [[6, 15, 20], [6, 15, 20]],
                sPaginationType: "full_numbers",
                "bAutoWidth": true,
                "sScrollX": "100%",
                "bScrollAutoCss":true,
                ...
                "sDom": 'T<"top"f>rt<"bottom"lpi><"clear">',
                "bServerSide": true,
                "sAjaxSource": "/ajax/get_products_list/",
                //"fnServerParams": function(aoData) {
                //    aoData.push({name:"sSearch_4",value:"172.29.142.136",});
                //},
                "fnServerData":function(sSource,aoData,fnCallback){
                ...
                "oTableTools":{
                    "sRowSelect": "single",             
                    "fnRowSelected":function(node){
                        $selectproduct_selectrow = $(node).children();
             ...
             $('#selectproducts_filter').attr('style','height:25px;');
        
            $("<a href='#' class='btn' id='selectproduct_btn' style='float:right;width:66px;margin-right:2px;'><i class='icon-edit'></i>选中</a> ").appendTo('#selectproducts_filter');

            
            $("#selectproduct_btn").bind("click", function (event) {
                event.preventDefault();
                if($selectproduct_selectrow == null){
                    $("#select-sale-dialog").dialog("open");
                }else{
                    $('#addproductid').val($($selectproduct_selectrow[0]).text());
                    $('#addproductname').val($($selectproduct_selectrow[1]).text());
                    $('#addproductspec').val($($selectproduct_selectrow[2]).text());
            ...
            //selectcustomer show
            var $selectcustomer_selectrow = null;
            
            $("#selectcustomers").dataTable({
                "iDisplayLength": 5,
                "aLengthMenu": [[5, 15, 20], [5, 15, 20]],
                sPaginationType: "full_numbers",
                "bAutoWidth": true,
                "sScrollX": "100%",
                "bScrollAutoCss":true,
                ...
                "oTableTools":{
                    "sRowSelect": "single",             
                    "fnRowSelected":function(node){
                        $selectcustomer_selectrow = $(node).children();
            ...
            $('#selectcustomers_filter').attr('style','height:25px;');
        
            $("<a href='#' class='btn' id='selectcustomer_btn' style='float:right;width:66px;margin-right:2px;'><i class='icon-edit'></i>选中</a> ").appendTo('#selectcustomers_filter');

            
            $("#selectcustomer_btn").bind("click", function (event) {
                event.preventDefault();
                if($selectcustomer_selectrow == null){
                    $("#select-sale-dialog").dialog("open");
                }else{
                    $('#addcustid').val($($selectcustomer_selectrow[0]).text());
                    $('#addcustcompany').val($($selectcustomer_selectrow[1]).text());
            ...
            //selectfeeder show
            var $selectfeeder_selectrow = null;
            
            $("#selectfeeders").dataTable({
                "iDisplayLength": 5,
                "aLengthMenu": [[5, 15, 20], [5, 15, 20]],
                sPaginationType: "full_numbers",
                "bAutoWidth": true,
                ...
                "bServerSide": true,
                //"sAjaxSource": "/ajax/get_students_list/",
                "sAjaxSource": "/ajax/get_feeders_list/",
                //"fnServerParams": function(aoData) {
                //    aoData.push({name:"sSearch_4",value:"172.29.142.136",});
                //},
                "fnServerData":function(sSource,aoData,fnCallback){
                ...
                "oTableTools":{
                    "sRowSelect": "single",             
                    "fnRowSelected":function(node){
                        $selectfeeder_selectrow = $(node).children();
            ...
            $('#selectfeeders_filter').attr('style','height:25px;');
        
            $("<a href='#' class='btn' id='selectfeeder_btn' style='float:right;width:66px;margin-right:2px;'><i class='icon-edit'></i>选中</a> ").appendTo('#selectfeeders_filter');

            
            $("#selectfeeder_btn").bind("click", function (event) {
                event.preventDefault();
                if($selectfeeder_selectrow == null){
                    $("#select-stock-dialog").dialog("open");
                }else{
                    $('#addfeederid').val($($selectfeeder_selectrow[0]).text());
                    $('#addfeedercompany').val($($selectfeeder_selectrow[1]).text());
            ...
            $("#feeders").dataTable({
                ...
                "oTableTools":{
                    "sRowSelect": "single",             
                    "fnRowSelected":function(node){
                        $feeder_selectrow = $(node).children();
            ...
            $('#feeders_filter').attr('style','height:25px;');
        
            $("<a href='#' class='btn' id='editfeeder' style='float:right;width:66px;margin-right:2px;'><i class='icon-edit'></i>修改</a> \
            <a href='#' class='btn' id='deletefeeder'  style='float:right;width:66px;margin-right:2px;'><i class='icon-remove'></i>注销</a> \
            <a href='#' class='btn' id='addfeeder' style='float:right;width:66px;margin-right:2px;'><i class='icon-plus'></i>添加</a>").appendTo('#feeders_filter');
            
            
            
            //delete one row feeder
            $("#deletefeeder").bind("click", function (event) {
                event.preventDefault();
                if($feeder_selectrow == null){
                    $("#select-feeder-dialog").dialog("open");
                }else{
                    $('#delete_feeder_id').val($($feeder_selectrow[0]).text());
                    $("#delete-feeder-dialog").dialog("open");
                }
            });

            /*add one feeder */
            $("#addfeeder").bind("click", function (event) {
                $("#add-feeder-dialog").dialog("open");
                event.preventDefault();
            });
            
            //edit one row feeder
            
            $("#editfeeder").bind("click", function (event) {
                event.preventDefault();
                if($feeder_selectrow == null){
                    $("#select-feeder-dialog").dialog("open");
                }else{
                    $('#editfeederid').val($($feeder_selectrow[0]).text());
                    $('#editfeedercompany').val($($feeder_selectrow[1]).text());
                    $('#editlinkman').val($($feeder_selectrow[2]).text());
                    $('#editaddress').val($($feeder_selectrow[3]).text());
                    $('#editcity').val($($feeder_selectrow[4]).text());
                    $('#editregion').val($($feeder_selectrow[5]).text());
                    $('#editzipcode').val($($feeder_selectrow[6]).text());
                    $('#edittel').val($($feeder_selectrow[7]).text());
                    $('#editfax').val($($feeder_selectrow[8]).text());
                    $('#edithomepage').val($($feeder_selectrow[9]).text());
                    $('#editmessage').val($($feeder_selectrow[10]).text());
                    
                    

                    $("#edit-feeder-dialog").dialog("open");
                }
            });
            ...
            $("#products").dataTable({
                ...
                "oTableTools":{
                    "sRowSelect": "single",             
                    "fnRowSelected":function(node){
                        $product_selectrow = $(node).children();
            ...
            $('#products_filter').attr('style','height:25px;');
        
            $("<a href='#' class='btn' id='editproduct' style='float:right;width:66px;margin-right:2px;'><i class='icon-edit'></i>修改</a> \
            <a href='#' class='btn' id='deleteproduct'  style='float:right;width:66px;margin-right:2px;'><i class='icon-remove'></i>注销</a> \
            <a href='#' class='btn' id='addproduct' style='float:right;width:66px;margin-right:2px;'><i class='icon-plus'></i>添加</a>").appendTo('#products_filter');
            
            //delete one row product
            $("#deleteproduct").bind("click", function (event) {
                event.preventDefault();
                if($product_selectrow == null){
                    $("#select-product-dialog").dialog("open");
                }else{
                    $('#delete_product_id').val($($product_selectrow[0]).text());
                    $("#delete-product-dialog").dialog("open");
                }
            });

            /*add one product */
            $("#addproduct").bind("click", function (event) {
                $("#add-product-dialog").dialog("open");
                event.preventDefault();
            });
            
            //edit one row product
            
            $("#editproduct").bind("click", function (event) {
                event.preventDefault();
                if($product_selectrow == null){
                    $("#select-product-dialog").dialog("open");
                }else{
                    $('#editproductid').val($($product_selectrow[0]).text());
                    $('#editproductname').val($($product_selectrow[1]).text());
                    $('#editproductspec').val($($product_selectrow[2]).text());
                    $('#editproductunit').val($($product_selectrow[3]).text());
                    $('#editmessage').val($($product_selectrow[4]).text());
            ...
            $("#hubs").dataTable({
                ...
                "sDom": 'T<"top"f>rt<"bottom"lpi><"clear">',
                "bServerSide": true,
                "sAjaxSource": "/ajax/get_hubs_list/",
                //"fnServerParams": function(aoData) {
                //    aoData.push({name:"sSearch_4",value:"172.29.142.136",});
                //},
                "fnServerData":function(sSource,aoData,fnCallback){
            ...
            $("#hubalarm").dataTable({
                ...
                "sDom": 'T<"top"f>rt<"bottom"lpi><"clear">',
                "bServerSide": true,
                "sAjaxSource": "/ajax/get_hubalarm_list/",
                //"fnServerParams": function(aoData) {
                //    aoData.push({name:"sSearch_4",value:"172.29.142.136",});
                //},
                "fnServerData":function(sSource,aoData,fnCallback){
                ...
                "oTableTools":{
                    "sRowSelect": "single",             
                    "fnRowSelected":function(node){
                        $hubalarm_selectrow = $(node).children();
            ...
            $("#profit").dataTable({
                ...
                "sDom": 'T<"top"f>rt<"bottom"lpi><"clear">',
                "bServerSide": true,
                "sAjaxSource": "/ajax/get_profit_list/",
                //"fnServerParams": function(aoData) {
                //    aoData.push({name:"sSearch_4",value:"172.29.142.136",});
                //},
                "fnServerData":function(sSource,aoData,fnCallback){
                ...
                "oTableTools":{
                    "sRowSelect": "single",             
                    "fnRowSelected":function(node){
                        $profit_selectrow = $(node).children();
            ...
            $("#sales").dataTable({
                ...
                "sDom": 'T<"top"f>rt<"bottom"lpi><"clear">',
                "bServerSide": true,
                "sAjaxSource": "/ajax/get_sales_list/",
                //"fnServerParams": function(aoData) {
                //    aoData.push({name:"sSearch_4",value:"172.29.142.136",});
                //},
                "fnServerData":function(sSource,aoData,fnCallback){
            ...
            $('#sales_filter').attr('style','height:25px;');
        
            $("<a href='#' class='btn' id='deletesale'  style='float:right;width:66px;margin-right:2px;'><i class='icon-remove'></i>注销</a> \
            <a href='#' class='btn' id='addsale' style='float:right;width:66px;margin-right:2px;'><i class='icon-plus'></i>增加</a>").appendTo('#sales_filter');

            /*add one sale */
            $("#addsale").bind("click", function (event) {
                $("#add-sale-dialog").dialog("open");
                event.preventDefault();
            });
            

            //delete one row sale
            $("#deletesale").bind("click", function (event) {
                event.preventDefault();
                if($sale_selectrow == null){
                    $("#select-sale-dialog").dialog("open");
                }else{
                    $('#delete_sale_id').val($($sale_selectrow[0]).text());
                    $("#delete-sale-dialog").dialog("open");
                }
            });
            ...
            /* add stock, select one feeder */
            $("#btnaddfeeder").bind("click", function (event) {
                $("#stock-select-feeder-dialog").dialog("open");
                event.preventDefault();
            });

            /* add sale, select one customer */
            $("#btnaddcustomer").bind("click", function (event) {
                $("#sale-select-customer-dialog").dialog("open");
                event.preventDefault();
            });

            /* add sale,stock,  select one product */
            $("#btnaddproduct").bind("click", function (event) {
                $("#sale-select-product-dialog").dialog("open");
                event.preventDefault();
            });
            ...
            $("#stocks").dataTable({
                ...
                "sDom": 'T<"top"f>rt<"bottom"lpi><"clear">',
                "bServerSide": true,
                "sAjaxSource": "/ajax/get_stocks_list/",
                //"fnServerParams": function(aoData) {
                //    aoData.push({name:"sSearch_4",value:"172.29.142.136",});
                //},
                "fnServerData":function(sSource,aoData,fnCallback){
            ...
            $('#stocks_filter').attr('style','height:25px;');
        
            $("<a href='#' class='btn' id='deletestock'  style='float:right;width:66px;margin-right:2px;'><i class='icon-remove'></i>注销</a> \
            <a href='#' class='btn' id='addstock' style='float:right;width:66px;margin-right:2px;'><i class='icon-plus'></i>增加</a>").appendTo('#stocks_filter');

           /*add one stock */
            $("#addstock").bind("click", function (event) {
                $("#add-stock-dialog").dialog("open");
                event.preventDefault();
            });
            

            //delete one row stock
            $("#deletestock").bind("click", function (event) {
                event.preventDefault();
                if($stock_selectrow == null){
                    $("#select-stock-dialog").dialog("open");
                }else{
                    $('#delete_stock_id').val($($stock_selectrow[0]).text());
                    $("#delete-stock-dialog").dialog("open");
                }
            });
